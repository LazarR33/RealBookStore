...
public class CommentRepository {
    ...
    public void create(Comment comment) {
        //LOSE String query = "insert into comments(bookId, userId, comment) values (" + comment.getBookId() + ", " + comment.getUserId() + ", '" + comment.getComment() + "')";
        String query = "insert into comments(bookId, userId, comment) values (?, ?, ?)";

        try (Connection connection = dataSource.getConnection();
             //LOSE Statement statement = connection.createStatement();
    ...
    public List<Comment> getAll(int bookId) {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            e.printStackTrace();
            LOG.warn("Error while getting comment list for bookId: " + bookId);
        }
        return commentList;
    }
}
